...
class captcha
{
	...
	function makeCaptcha ()
	{
		...
		$imagelength = $this->Length * 45 + 36;			//CHANGED BY WALTER CERRUDO 9/11/2010
		$imageheight = 75;
		...
		for ($i = 0; $i < strlen($this->CaptchaString); $i++)
		{
			...
			imagettftext($image, 35, mt_rand(-15, 15), $i * 45 + 10,											//CHANGED BY WALTER CERRUDO 9/11/2010
			mt_rand(40, 70),
			$stringcolor,
			$this->getRandFont(),
		...
		//$filter->noise($image, 40);
		//$filter->blur($image, 4);

		imagecolortransparent($image,hexdec('FFFFFF'));					//ADDED BY WALTER CERRUDO 9/11/2010
		imagepng($image,'./tmp/captcha.png');							//COMMENT BY WALTER CERRUDO 9/11/2010
